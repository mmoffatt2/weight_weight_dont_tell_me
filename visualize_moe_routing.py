...
def plot_expert_heatmap(expert_counts, model_name, save_dir, dataset_name):
    ...
    plt.title(f"Expert Usage per Layer\nModel: {model_name}\nDataset: {dataset_name}", fontsize=16, pad=14)
...
def plot_expert_bar(expert_counts, model_name, save_dir, dataset_name):
    ...
    plt.xlabel("Expert ID", fontsize=13)
    plt.ylabel("Total tokens routed", fontsize=13)
    plt.title(f"Global Expert Load\nModel: {model_name}\nDataset: {dataset_name}", fontsize=16, pad=14)
    plt.tight_layout()
...
def plot_entropy_trace(token_trace, model_name, save_dir, dataset_name):
    ...
    plt.ylabel("Average Routing Entropy", fontsize=13)
    plt.title(f"Routing Sharpness per Layer\nModel: {model_name}\nDataset: {dataset_name}", fontsize=16, pad=14)
    plt.tight_layout()
...
def plot_expert_usage_count(expert_counts, model_name, save_dir, dataset_name):
    ...
    plt.title(f"Absolute Expert Usage per Layer\nModel: {model_name}\nDataset: {dataset_name}", fontsize=16, pad=14)
    plt.xlabel("Expert ID", fontsize=13)
    plt.ylabel("Layer", fontsize=13)
    plt.tight_layout()
...
def plot_expert_sparsity(expert_counts, model_name, save_dir, dataset_name):
    """Plot sparsity (log-normalized expert usage)."""
    counts_norm = expert_counts / (expert_counts.sum(dim=1, keepdim=True) + 1e-8)
    ...
    plt.title(f"Expert Sparsity Map (Low = Underused)\nModel: {model_name}\nDataset: {dataset_name}", fontsize=16, pad=14)
    plt.xlabel("Expert ID", fontsize=13)
    plt.ylabel("Layer", fontsize=13)
    plt.tight_layout()
...
def main(args):
    ...
    plot_expert_heatmap(expert_counts, args.model_name, args.results_dir, args.dataset_name)
    plot_expert_bar(expert_counts, args.model_name, args.results_dir, args.dataset_name)
    plot_expert_usage_count(expert_counts, args.model_name, args.results_dir, args.dataset_name)
    plot_expert_sparsity(expert_counts, args.model_name, args.results_dir, args.dataset_name)

    if token_trace:
        plot_entropy_trace(token_trace, args.model_name, args.results_dir, args.dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, required=True, help="Dataset name used during tracing")
